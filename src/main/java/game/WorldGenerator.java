...
//    Random random = new Random(subworld.seed);
    ...
    Chunk generateChunk(VectorI indexes) {
        ...
        Material water = Content.getMaterial("water");
        int i = 0;
        for (int y = 0; y < Chunk.size(); y++)
            for (int x = 0; x < Chunk.size(); x++) {
                float noise = OpenSimplex2S.noise2(subworld.seed, (baseX + x) * 0.01, (baseY + y) * 0.01);
                float waterNoise = OpenSimplex2S.noise2(subworld.seed + 1, (baseX + x) * 0.01, (baseY + y) * 0.01);
                float factor = noise + (baseY + y + 60) * 0.003f;
                float waterFactor = waterNoise - (baseY + y -    60) * 0.003f;
                if (abs(baseX) > 600)
                ...
                } else if (waterFactor > 0.8) {
                    chunk.presetPixel(i, water, (byte) -1);
                } else {
